...
import '../../../shared/models/sayfali_yanit.dart';
import '../../../shared/models/yorum_model.dart';
import '../../../shared/sayfalama/sayfalama_yardimcilari.dart';
import '../providers/profil_provider.dart';

class ProfilIcerikSekmeleri extends ConsumerStatefulWidget {
  final int userId;
  final bool isOwnProfile;

  const ProfilIcerikSekmeleri({
    super.key,
  ...
  @override
  ConsumerState<ProfilIcerikSekmeleri> createState() =>
      _ProfilIcerikSekmeleriDurumu();
}

class _ProfilIcerikSekmeleriDurumu extends ConsumerState<ProfilIcerikSekmeleri>
    with SingleTickerProviderStateMixin {
  late final TabController _tabController;
  // Merkezi Dio konfigürasyonu paylaşılsın diye ApiServisi Riverpod'dan alınır.
  late final ApiServisi _api;

  @override
  void initState() {
    super.initState();
    _api = ref.read(profilApiProvider);
        ...
        children: [
          TabBar(
            ...
            tabAlignment: tabs.length > 2
                ? TabAlignment.start
                : TabAlignment.fill,
            labelColor: UygulamaTemasi.anaRenk,
            indicatorColor: UygulamaTemasi.anaRenk,
          ...
          SizedBox(
            ...
            child: TabBarView(
              controller: _tabController,
              children: [
                _SayfaliListeGorunumu<IcerikModel>(
                  loader: (page) =>
                      _api.profilIcerikleriniGetir(widget.userId, page: page),
                ...
                if (widget.isOwnProfile)
                  _SayfaliListeGorunumu<IcerikModel>(
                    loader: (page) => _api.profilKaydedilenleriGetir(
                      widget.userId,
                      page: page,
                    ),
                ...
                if (widget.isOwnProfile)
                  _SayfaliListeGorunumu<YorumOzetModel>(
                    loader: (page) =>
                        _api.profilBegenileriniGetir(widget.userId, page: page),
...
class _SayfaliListeGorunumu<T> extends StatefulWidget {
  final Future<SayfaliYanit<T>> Function(int page) loader;
  final Widget Function(BuildContext, T) itemBuilder;
  final String emptyText;

  const _SayfaliListeGorunumu({
  ...
  @override
  State<_SayfaliListeGorunumu<T>> createState() =>
      _SayfaliListeGorunumuDurumu<T>();
}

class _SayfaliListeGorunumuDurumu<T> extends State<_SayfaliListeGorunumu<T>>
  ...
  void _onScroll() {
    if (_sayfalamaTetikleyici.yuklemeliMi(_scrollController, esik: 240)) {
      _dahaYukle();
    }
  }
  ...
  Future<void> _dahaYukle() async {
    ...
    try {
      ...
      setState(() {
        _page += 1;
        _items.addAll(response.sonuclar);
        _dahaVar = response.sonrakiVarMi;
      });
    } catch (_) {
