...
// SORU SORMA EKRANI
// ---------------------------------------------------------------------------
// Kullanıcının foruma yeni bir soru gönderdiği ekran. Başlık, açıklama ve
// kategori alınır; isteğe bağlı olarak bir görsel de eklenebilir.
// ---------------------------------------------------------------------------
...
/// Foruma yeni soru/içerik gönderme ekranı.
class SoruSorEkrani extends StatefulWidget {
  const SoruSorEkrani({super.key});

  @override
  State<SoruSorEkrani> createState() => _SoruSorEkraniDurumu();
}

class _SoruSorEkraniDurumu extends State<SoruSorEkrani> {
  ...
  Future<void> _kategorileriYukle() async {
    ...
    try {
      final kategoriler = await _api.kategorileriGetir();
      if (!mounted) return;
      setState(() {
