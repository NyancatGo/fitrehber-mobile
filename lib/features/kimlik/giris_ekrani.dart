...
// GİRİŞ EKRANI
// ---------------------------------------------------------------------------
// Kullanıcının uygulamaya giriş yaptığı ekran. İki giriş yöntemi sunar:
//   1) Kullanıcı adı + parola ile klasik giriş.
//   2) Google hesabıyla giriş (OAuth).
//
// Form, gönderilmeden önce `Form` + `validator` ile doğrulanır; parola alanı
// gizlenebilir (maskeleme); hatalar kullanıcıya anlaşılır Türkçe mesajla
// gösterilir.
// ---------------------------------------------------------------------------
...
import '../../shared/google_oauth_akisi.dart';
import '../../shared/hata_yardimcilari.dart';
import '../../shared/kimlik_servisi.dart';
import '../../shared/oturum_denetleyici.dart';
import '../../shared/widgets/oturum_yukleme.dart';

/// Kullanıcı adı/parola veya Google ile giriş yapılan ekran.
...
class _GirisEkraniDurumu extends ConsumerState<GirisEkrani> {
  ...
  /// Google girişinin deep link dönüşünü dinlemek için kullanılır.
  final _appLinks = AppLinks();

  /// Klasik giriş işlemi sürüyor mu?
  bool _isLoading = false;

  /// Google ile giriş işlemi sürüyor mu?
  ...
  /// Herhangi bir giriş işlemi sürerken `true` döner; butonları kilitlemek için.
  ...
  ///
  /// Form geçerli değilse hiçbir şey yapmaz. Hesabın e-postası doğrulanmamışsa
  /// kullanıcı doğrulama ekranına yönlendirilir; diğer hatalar ekranda gösterilir.
  Future<void> _girisYap() async {
    // Form alanları geçerli değilse isteği hiç göndermeyelim.
    ...
    try {
      await ref
          ...
          .girisYap(_usernameController.text.trim(), _passwordController.text);
    } on EpostaDogrulamasiGerekliHatasi catch (e) {
      // Hesap var ama e-posta doğrulanmamış: doğrulama ekranına gönder.
      if (mounted) {
  ...
  /// Google hesabıyla giriş akışını başlatır.
  ///
  /// Web ve mobil platformlar farklı çalışır: web'de tüm sayfa yönlendirilir,
  /// mobilde tarayıcı açılıp deep link ile geri dönülür.
  Future<void> _googleIleGirisYap() async {
    final oauthRequest = GoogleOAuthAkisi.istekOlustur();
    ...
    // Mobil akış: Google'dan dönen deep link'i dinleyip kodu yakalarız.
    StreamSubscription<Uri>? linkSubscription;
    final callbackCompleter = Completer<GoogleOAuthDonusu>();

    try {
      linkSubscription = _appLinks.uriLinkStream.listen(
        (uri) {
          if (uri.scheme != GoogleOAuthAkisi.donusSemasi ||
  ...
  /// yüklenince OturumDenetleyici.oturumuGeriYukle() token exchange'i tamamlar.
  Future<void> _googleIleGirisYapWeb(GoogleOAuthIstegi oauthRequest) async {
    try {
      await ref
          .read(kimlikServisiProvider)
          .bekleyenGoogleIstekKaydet(
