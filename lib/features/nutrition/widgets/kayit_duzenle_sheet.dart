...
/// Mevcut bir öğün kaydının gramajını düzenlemek veya kaydı silmek için
/// kullanılan bottom sheet. Backend'de update endpoint olmadığı için
/// gramaj değişikliği "sil + yeniden ekle" workflow'u ile yapılır.
...
class _KayitDuzenleSheetState extends ConsumerState<_KayitDuzenleSheet> {
  ...
  Future<void> _kaydet() async {
    ...
    final ok = await ref.read(nutritionProvider.notifier).ogunGuncelle(
          eski: widget.kayit,
          yeniMiktar: _currentGram,
        );
    if (!mounted) return;
    ...
    if (ok) {
      ...
      final hata = ref.read(nutritionProvider).hata ?? 'Güncellenemedi.';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(hata),
  ...
  Future<void> _sil() async {
    ...
    if (ok) {
      ...
      final hata = ref.read(nutritionProvider).hata ?? 'Silinemedi.';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(hata),
          backgroundColor: const Color(0xFFDC2626),
