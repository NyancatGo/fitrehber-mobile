...
import 'yorum_iskeleti.dart';
import 'baglantili_metin.dart';
...
class YorumKarti extends StatefulWidget {
  ...
  @override
  State<YorumKarti> createState() => _YorumKartiDurumu();
}

class _YorumKartiDurumu extends State<YorumKarti> {
  ...
  Future<void> _fetchReplies() async {
    ...
    try {
      final apiServisi = ApiServisi();
      final yanitlar = await apiServisi.yanitlariGetir(widget.yorum.id);
      if (!mounted) return;
      setState(() {
        _replies = yanitlar;
        _hasCheckedApi = true;
                  ...
                  child: Container(
                    ...
                    child: _isLoadingReplies
                        ? const Padding(
                            ...
                            child: YorumIskeletListesi(adet: 2),
                          )
                        : Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _kart(
    ...
    final bg = _isOp
        ? UygulamaTemasi.anaRenk.withValues(alpha: 0.07)
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: _isOp
              ? UygulamaTemasi.anaRenk.withValues(alpha: 0.30)
      ...
      child: IntrinsicHeight(
        child: Row(
          ...
          children: [
            if (_isOp) Container(width: 3.5, color: UygulamaTemasi.anaRenk),
            Expanded(
              child: Padding(
  ...
  Widget _opRozeti() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: UygulamaTemasi.anaRenk.withValues(alpha: 0.18),
        borderRadius: BorderRadius.circular(5),
        border: Border.all(
          color: UygulamaTemasi.anaRenk.withValues(alpha: 0.45),
        ),
      ),
      child: const Text(
        'YAZAR',
