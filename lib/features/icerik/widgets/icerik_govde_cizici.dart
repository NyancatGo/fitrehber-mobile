// ---------------------------------------------------------------------------
// HTML MAKALE İÇERİĞİ ÇİZİCİSİ (ESKİ YÖNTEM)
// ---------------------------------------------------------------------------
// Makale gövdesini ham HTML'den çizer. Backend yeni "mobil blok" çıktısını
// (`yazi_mobil_bloklar`) gönderdiğinde `IcerikBlokCizici` kullanılır;
// bu çizici ise blok verisi yoksa devreye giren geriye dönük (fallback) yoldur.
// ---------------------------------------------------------------------------
...
/// Ham HTML makale içeriğini Flutter widget'larına dönüştürerek çizen widget.
class IcerikGovdeCizici extends StatelessWidget {
  final String html;
  final double contentWidth;

  const IcerikGovdeCizici({
    super.key,
    required this.html,
  ...
  Widget build(BuildContext context) {
    ...
    final renderer = _IcerikCizici(width: width);
...
class _IcerikCizici {
  ...
  _IcerikCizici({required this.width});

  bool get isCompact => width < 640;
  bool get isWide => width >= 700;

  TextStyle get bodyStyle => TextStyle(
    color: _IcerikRenkleri.text,
  ...
  TextStyle get leadStyle => bodyStyle.copyWith(
    color: _IcerikRenkleri.textStrong,
    fontSize: isCompact ? 17 : 18,
    height: 1.68,
  );

  TextStyle get mutedStyle => bodyStyle.copyWith(
    color: _IcerikRenkleri.muted,
  ...
  Widget? _renderNode(dom.Node node, {bool tight = false}) {
    ...
    if (_hasClass(node, 'box-summary')) {
      return _accentPanel(
        ...
        color: _IcerikRenkleri.primarySoft,
        accent: _IcerikRenkleri.primary,
    ...
    if (_hasClass(node, 'highlight-warn')) {
      return _accentPanel(
        child: _childrenColumn(node, tight: true),
        color: _IcerikRenkleri.warnSoft,
        accent: _IcerikRenkleri.warn,
      );
    }
  ...
  Widget _toc(dom.Element element) {
    ...
    return _accentPanel(
      ...
      color: _IcerikRenkleri.surfaceSoft,
      accent: _IcerikRenkleri.primary,
  ...
  Widget _fractionItem(dom.Element element) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _IcerikRenkleri.line),
      ),
      child: Row(
  ...
  Widget _paragraph(dom.Element element, {bool tight = false}) {
    ...
    final style = isTitle
        ...
        : isLink
        ? bodyStyle.copyWith(
            color: _IcerikRenkleri.link,
            fontWeight: FontWeight.w800,
            decoration: TextDecoration.underline,
            decorationColor: _IcerikRenkleri.link,
          )
        : isMeta
          ...
          SizedBox(
            ...
            child: Text(
              ...
              style: bodyStyle.copyWith(
                color: ordered
                    ? _IcerikRenkleri.text
                    : _IcerikRenkleri.textStrong,
      ...
      child: AspectRatio(
        ...
        child: Image.network(
          ...
          errorBuilder: (context, error, stackTrace) => Container(
            ...
            decoration: BoxDecoration(
              color: _IcerikRenkleri.surfaceElevated,
  ...
  List<InlineSpan> _inlineSpans(Iterable<dom.Node> nodes, TextStyle style) {
    ...
    for (final node in nodes) {
      ...
      if (tag == 'strong' || tag == 'b') {
        ...
        childStyle = childStyle.copyWith(
          color: _IcerikRenkleri.textStrong,
          backgroundColor: _IcerikRenkleri.surfaceElevated,
