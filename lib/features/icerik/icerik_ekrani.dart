...
// MAKALE DETAY EKRANI
// ---------------------------------------------------------------------------
// Tek bir makalenin/içeriğin tam görünümü: kapak görseli, başlık, yazar,
// makale gövdesi ve altında yorumlar bölümü. Kullanıcı buradan makaleyi
// beğenebilir, kaydedebilir ve yorum yazabilir.
// ---------------------------------------------------------------------------
...
import 'widgets/icerik_blok_cizici.dart';
import 'widgets/icerik_govde_cizici.dart';
import 'widgets/yorum_giris_cubugu.dart';
import 'widgets/yorum_iskeleti.dart';
import 'widgets/yorum_karti.dart';

/// Bir makalenin tüm detayını ve yorumlarını gösteren ekran.
class IcerikEkrani extends ConsumerStatefulWidget {
  /// Gösterilecek makalenin kimliği.
  final int id;
  const IcerikEkrani({super.key, required this.id});

  @override
  ConsumerState<IcerikEkrani> createState() => _IcerikEkraniDurumu();
}

class _IcerikEkraniDurumu extends ConsumerState<IcerikEkrani> {
  ...
  Future<void> _icerikYukle() async {
    ...
    try {
      final icerik = await _api.icerikDetayiniGetir(widget.id);
      if (!mounted) return;
  ...
  Future<void> _yorumlariYukle() async {
    ...
    try {
      final duzListe = await _api.yorumlariGetir(widget.id);
      if (!mounted) return;
  ...
  Future<void> _icerikBegen() async {
    ...
    try {
      final sonuc = await _api.icerikBegenisiniDegistir(mevcut.id);
      if (!mounted) return;
      final sayi = sonuc['begeni_sayisi'];
      setState(() {
        _icerik = _icerik?.copyWith(
  ...
  Future<void> _icerikKaydet() async {
    ...
    try {
      final sonuc = await _api.icerikKaydiniDegistir(mevcut.id);
      if (!mounted) return;
      setState(() {
    ...
    if (_yorumlarYukleniyor) {
      return [
        SliverToBoxAdapter(
          child: _ortala(
            padding: altPadding,
            child: const YorumIskeletListesi(),
          ),
        ),
      ];
    }
